...
router.get("/summary", async (req, res) => {
    try {
        ...
        const leaveQuery = `
            SELECT
                COUNT(*) FILTER (
                    WHERE LOWER(leave_type) LIKE '%sick%'
                ) AS sick,

                COUNT(*) FILTER (
                    WHERE
                        LOWER(leave_type) LIKE '%family%'
                        OR LOWER(leave_type) LIKE '%personal%'
                ) AS family,

                COUNT(*) FILTER (
                    WHERE
                        LOWER(leave_type) NOT LIKE '%sick%'
                        AND LOWER(leave_type) NOT LIKE '%family%'
                        AND LOWER(leave_type) NOT LIKE '%personal%'
                ) AS other,
        ...
        try {
            ...
            recentReports = reportsResult.rows.map(
                (report) => ({
                    id: report.id,
                    name: report.name,
                    type: report.type,
...
router.get("/:id/download", async (req, res) => {
    try {
        ...
        // Later we can replace this with a real PDF/XLSX generator.
